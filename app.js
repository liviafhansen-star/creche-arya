...
function isCrecheRole() {
  ...
  if (typeof rememberedAuthRole === "function" && rememberedAuthRole() === "creche") return true;
  return false;
}
...
async function loadAll() {
  ...
  try {
    ...
    if (!restoring && !isCrecheAccountUser(user) && lockedRole !== wantedRole) {
      ...
      if (errEl) { errEl.textContent = roleMismatchMessage(lockedRole); errEl.classList.remove("hidden"); }
      return;
    }
    rememberAuthRole(lockedRole);
    if (lockedRole === "creche" && typeof ensureClientsShape === "function") {
      ensureClientsShape();
      if (typeof getClients === "function" && !getClients().length) seedDemoCrecheClients();
    }
...
function renderCrecheTutores() {
  if (!isCrecheRole()) return;
  if (typeof ensureClientsShape === "function") ensureClientsShape();
  const el = document.getElementById("crecheTutoresList");
  if (!el) return;
  const clients = typeof getClients === "function" ? getClients() : [];
  if (!clients.length) { el.innerHTML = "<div class='trip'>Nenhum tutor ainda. Cadastre cães em Cães.</div>"; return; }
  const byTutor = {};
  clients.forEach(c => {
    const key = (c.tutor_name || "Sem nome").trim() || "Sem nome";
    if (!byTutor[key]) byTutor[key] = { phone: c.tutor_phone || "", dogs: [] };
    byTutor[key].dogs.push(c.name || "Cão");
    if (c.tutor_phone) byTutor[key].phone = c.tutor_phone;
  });
  el.innerHTML = Object.keys(byTutor).map(name => {
    const t = byTutor[name];
    return `<div class="trip"><div><b>${name}</b><small>${t.dogs.join(", ")}</small></div><span class="amount" style="font-size:12px;font-weight:600">${t.phone || "—"}</span></div>`;
  }).join("");
}
function renderAll() { applyRoleLayout(); renderHome(); renderOpenMonths(); renderCalendar(); renderProfile(); renderPayments(); renderSavedMessages(); renderCrecheToday(); renderCrecheFamilies(); renderCrecheTutores(); renderCrecheProfile(); updateChromeNames(); updateBrandForRole(); const mt = document.getElementById("messageText"); if (mt) mt.value = data.message || ""; updateQuickLabels(); }
...
function updateNavForRole() {
  ...
  const calBtn = document.querySelector('#appNav button[data-screen="calendar"], #appNav button[data-screen="crecheTutores"]');
  if (creche) {
    if (homeL) homeL.textContent = "Hoje";
    if (calBtn) {
      calBtn.dataset.screen = "crecheTutores";
      calBtn.onclick = function () { go("crecheTutores", { fromNav: true }); };
      calBtn.innerHTML = '👥<span id="navCalendarLabel">Tutores</span>';
    }
    if (msgBtn) {
      msgBtn.dataset.screen = "crecheFamilies";
      msgBtn.onclick = function () { go("crecheFamilies", { fromNav: true }); };
      msgBtn.innerHTML = '🐶<span id="navMessagesLabel">Cães</span>';
    }
    if (payL) payL.textContent = "Financeiro";
  } else {
    if (homeL) homeL.textContent = "Início";
    if (calBtn) {
      calBtn.dataset.screen = "calendar";
      calBtn.onclick = function () { go("calendar", { fromNav: true }); };
      calBtn.innerHTML = '📅<span id="navCalendarLabel">Calendário</span>';
    }
    if (msgBtn) {
      msgBtn.dataset.screen = "messages";
      msgBtn.onclick = function () { go("messages", { fromNav: true }); };
...
function seedDemoCrecheClients() {
  if (!data.profile) return;
  const demo = [
    { id: "demo-arya", name: "Arya", tutor_name: "Lívia", tutor_phone: "(51) 99999-0001", notes: "", weekdays: [2, 3] },
    { id: "demo-thor", name: "Thor", tutor_name: "Maiquel", tutor_phone: "(51) 99999-0002", notes: "", weekdays: [1, 2, 3, 4, 5] },
    { id: "demo-luna", name: "Luna", tutor_name: "Ana", tutor_phone: "(51) 99999-0003", notes: "", weekdays: [2, 4] }
  ];
...
function isCrecheAccountUser(user) {
  return !!(user && user.email && typeof isCrecheSyntheticEmail === "function" && isCrecheSyntheticEmail(user.email));
}
function rememberedAuthRole() {
  try {
    return sessionStorage.getItem("creche_auth_role") || localStorage.getItem("creche_pending_role") || pendingRole || "tutor";
  } catch (e) { return pendingRole || "tutor"; }
}
function rememberAuthRole(role) {
